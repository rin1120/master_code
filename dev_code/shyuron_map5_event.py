...
# ★「まとめて更新」の設計
EVENT_WINDOW = 50   # 更新を溜める要求数
EVAL_WINDOW  = 50   # 評価を取る要求数
NUM_CYCLES   = 1     # (イベント窓→更新)→(評価窓→更新) を何セット回すか
...
def apply_icn_update(cache_storage, net_vector_arrays, cid, vect, start_node, path_from_found_to_start):
    ...
    # 3) update winner map along found->...->start path
...
# ----------------------------------------------------------------------------
def generate_multi_contents_tasks_r(size, k):
    tasks = []
    for _ in range(k):
...
def run_single_task_single_pheromone(cache_storage, net_vector_array, size, cid, start_node, log_list, pheromone_dict):
    ...
    # if content not present at all (should not happen with your placement, but keep safe)
    ...
    for t in range(NUM_ITERATIONS):
                ...
                for _ in range(TIMES_TO_SEARCH_HOP):
                    ...
                    if USE_EPSILON and random.random() < EPSILON:
                        ...
                        if NUM_ITERATIONS >= 2:
                            ...
                            ALPHA, BETA = ALPHA_START, BETA_START
        ...
        # iteration best (for local-best evaporation logic)
        if all_costs:
...
# ----------------------------------------------------------------------------
# Window runner (NO ICN update during window; apply later in batch)
# ----------------------------------------------------------------------------
def run_window_no_update(
    tasks,
    size,
    num_maps,
    cache_for_single,
    ...
    cache_for_attrib,
    vectors_for_attrib,
    single_pheromone_dict_list,
    attrib_pheromone_trails_list,
    debug_logs,
):
    """
    1つの窓（例: 100要求）を処理する。
    - 窓の中では更新しない
    - 各タスクの best_path / best_hops を集めて返す（窓終了後にまとめて更新するため）
    """
    # update buffers (for batch ICN update)
    updates_single = []  # list of (cid, start_node, vect, best_path)
    updates_attrib = []

    # metrics (best_hops per task)
    hops_single_list = []
    hops_attrib_list = []

    for (cid, start_node) in tasks:
        vect = cont_vector_array[cid - 1]
        ...
        hops_single_list.append(best_hops_single)
        if best_path_single is not None:
            updates_single.append((cid, start_node, vect, best_path_single))
        ...
        hops_attrib_list.append(best_hops_attrib)
        if best_path_attrib is not None:
            updates_attrib.append((cid, start_node, vect, best_path_attrib))

    return updates_single, hops_single_list, updates_attrib, hops_attrib_list

def apply_updates_batch(cache_storage, net_vector_arrays, updates):
    """
    updates: list of (cid, start_node, vect, best_path_found_to_start)
    """
    for (cid, start_node, vect, best_path) in updates:
        apply_icn_update(cache_storage, net_vector_arrays, cid, vect, start_node, best_path)
...
# ----------------------------------------------------------------------------
# One experiment for a given num_maps (batch update setting)
# ----------------------------------------------------------------------------
def run_experiment(num_maps, size=50, seed=None):
    if seed is not None:
        np.random.seed(seed)
        random.seed(seed)

    # init env (cache placement + SOM init)
    init_cache_storage, init_net_vector_arrays = cache_prop(size, num_maps)

    debug_logs = []

    # deep copy env for each method
    ...
    # evaluation metrics pooled across cycles (only EVAL window counts)
    eval_hops_single_all = []
    eval_hops_attrib_all = []

    for _cycle in range(NUM_CYCLES):
        # ---------------------------
        # (A) EVENT WINDOW (no update during)
        # ---------------------------
        event_tasks = generate_multi_contents_tasks_r(size, EVENT_WINDOW)

        upd_s, _, upd_a, _ = run_window_no_update(
            event_tasks, size, num_maps,
            cache_for_single, vectors_for_single,
            cache_for_attrib, vectors_for_attrib,
            single_pheromone_dict_list,
            attrib_pheromone_trails_list,
            debug_logs
        )

        # batch apply after the window
        apply_updates_batch(cache_for_single, vectors_for_single, upd_s)
        apply_updates_batch(cache_for_attrib, vectors_for_attrib, upd_a)

        # ---------------------------
        # (B) EVAL WINDOW (measure here, no update during)
        # ---------------------------
        eval_tasks = generate_multi_contents_tasks_r(size, EVAL_WINDOW)

        upd_s2, hops_s2, upd_a2, hops_a2 = run_window_no_update(
            eval_tasks, size, num_maps,
            cache_for_single, vectors_for_single,
            cache_for_attrib, vectors_for_attrib,
            single_pheromone_dict_list,
            attrib_pheromone_trails_list,
        ...
        # record metrics (EVAL only)
        eval_hops_single_all.extend(hops_s2)
        eval_hops_attrib_all.extend(hops_a2)

        # apply eval updates (prepare next cycle)
        apply_updates_batch(cache_for_single, vectors_for_single, upd_s2)
        apply_updates_batch(cache_for_attrib, vectors_for_attrib, upd_a2)

    result = {
        "num_maps": num_maps,
        "single_avg_best_hops": avg(eval_hops_single_all),
        "single_success_rate": succ_rate(eval_hops_single_all),
        "attrib_avg_best_hops": avg(eval_hops_attrib_all),
        "attrib_success_rate": succ_rate(eval_hops_attrib_all),
    }
    return result
...
# ----------------------------------------------------------------------------
# main: sweep num_maps = 1..5, average over TIME_TO_SIMULATE runs
# ----------------------------------------------------------------------------
def main():
    size = 50
    ...
    for num_maps in range(1, 6):
        ...
        rs = []
        for sim in range(TIME_TO_SIMULATE):
            r = run_experiment(num_maps=num_maps, size=size, seed=None)
            rs.append(r)

        # aggregate
        single_hops = [d["single_avg_best_hops"] for d in rs]
        single_succ = [d["single_success_rate"] for d in rs]
        attrib_hops = [d["attrib_avg_best_hops"] for d in rs]
        attrib_succ = [d["attrib_success_rate"] for d in rs]

        out = {
            "num_maps": num_maps,
            "single_avg_best_hops": float(np.mean(single_hops)),
            "single_success_rate": float(np.mean(single_succ)),
            "attrib_avg_best_hops": float(np.mean(attrib_hops)),
            "attrib_success_rate": float(np.mean(attrib_succ)),
        }
        all_results.append(out)

        print(f"\n=== maps={num_maps} ===")
        print(f"[SINGLE] avg best hops: {out['single_avg_best_hops']:.2f}, success: {out['single_success_rate']:.2f}%")
        print(f"[ATTRIB] avg best hops: {out['attrib_avg_best_hops']:.2f}, success: {out['attrib_success_rate']:.2f}%")

    df_sum = pd.DataFrame(all_results)
    df_sum.to_csv("summary_maps_1to5_batch_update.csv", index=False, encoding="utf-8-sig")
    print("\n>>> summary saved: summary_maps_1to5_batch_update.csv")
