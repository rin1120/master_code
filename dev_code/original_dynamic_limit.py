...
# ----------------------------------------------------------------------------
# ② 属性フェロモン方式 共通処理
# reset_pheromone=True: リセットあり、False: リセットなし（帯域変動）
            ...
            for path, cost in zip(all_paths, all_costs):
                if cost > 0:
                    ...
                    for i in range(len(path) - 1):
                        ...
                        pheromone_trails[edge] += delta
                        if not reset_pheromone:
                            tau_max = DYNAMIC_K * Q / best_cost
                            pheromone_trails[edge] = np.minimum(pheromone_trails[edge], tau_max)
            iteration_data.append(iter_costs)
        results.append(iteration_data)
    return results
...
def main():
    ...
    df.to_csv("dynamic_limit_debug_log.csv", index=False, encoding="utf-8-sig")
    print(f"\n>>> {len(df)} 行を書き出しました → dynamic_limit_debug_log.csv")
