# 既存手法 + 4つの手法（ACO、多次元ACO（リセットあり／なし（オリジナル／e-greedy適応））
...
def multi_contents_attrib_pheromone_common(cache_storage, net_vector_array, size, content_tasks, log_list, reset_pheromone=True):
    results = []
    tag = "ATTRIB_R" if reset_pheromone else "ATTRIB_NR_ADAPT"
...
# ----------------------------------------------------------------------------
# ③ 属性フェロモン方式 (リセットあり)　デフォルトバージョン
# ----------------------------------------------------------------------------
def multi_contents_attrib_pheromone_base(cache_storage, net_vector_array, size, content_tasks, log_list):
    results = []
    tag = "ATTRIB_NR_BASE"
    pheromone_trails = initialize_pheromone_trails(size, N)
    ...
    for (cid, start_node) in content_tasks:
        vect = cont_vector_array[cid - 1]
        iteration_data = []
        best_cost = TIMES_TO_SEARCH_HOP  # 各コンテンツ探索開始時に初期化
        for _ in range(NUM_ITERATIONS):
            iter_costs = []
            all_paths = []
            all_costs = []
...
def gather_stats_for_content(content_index, single_all_runs, attrib_reset_all_runs, attrib_noreset_all_adapt, attrib_noreset_all_base):
    ...
    an_b_stat = average_iteration_data_across_runs(attrib_noreset_all_base, content_index)
    return (s_stat, ar_stat, an_a_stat, an_b_stat)

def plot_four_metrics_for_content(stat_single, stat_attrib_reset, stat_attrib_noreset_adpat, stat_attrib_noreset_base, content_label):
    if (stat_single is None) or (stat_attrib_reset is None) or (stat_attrib_noreset_adpat is None) or (stat_attrib_noreset_base is None):
    ...
    an_b_med, an_b_q1, an_b_q3, an_b_mean, an_b_succ = stat_attrib_noreset_base
    ...
    plt.plot(its, an_a_med, label='Attrib(NR‑adapt)', color='green', marker='^')
    plt.fill_between(its, an_a_q1, an_a_q3, color='green', alpha=0.2)
    plt.plot(its, an_b_med, label='Attrib(NR‑base)', color='purple', marker='x')
    plt.fill_between(its, an_b_q1, an_b_q3, color='purple', alpha=0.2)
    ...
    plt.plot(its, an_a_succ, label='Attrib(NR‑adapt)', color='green', marker='^')
    plt.plot(its, an_b_succ, label='Attrib(NR‑base)', color='purple', marker='x')
    ...
    plt.plot(its, an_a_mean, label='Attrib(NR‑Adapt', color='green', marker='^')
    plt.plot(its, an_b_mean, label='Attrib(NR‑Base)', color='purple', marker='x')
...
def plot_overall_metrics(stats_dict):
    ...
    plt.plot(its, stats_dict['NR-Adapt'][0], label='NR-Adapt', color='green', marker='^')
    plt.plot(its, stats_dict['NR-Base'][0], label='NR-Base', color='purple', marker='x')
    ...
    plt.plot(its, stats_dict['NR-Adapt'][3], label='NR-adapt', color='green', marker='^')
    plt.plot(its, stats_dict['NR-Base'][3], label='NR-base', color='purple', marker='x')
    ...
    plt.plot(its, stats_dict['NR-Adapt'][4], label='NR-adapt', color='green', marker='^')
    plt.plot(its, stats_dict['NR-Base'][4], label='NR-base', color='purple', marker='x')
...
def main():
    ...
    for _sim in range(TIME_TO_SIMULATE):
        ...
        attrib_noreset_all_base_res = multi_contents_attrib_pheromone_base(cache_storage, net_vector_array, size, content_tasks, debug_logs)
        ...
        attrib_noreset_all_base.append(attrib_noreset_all_base_res)
    ...
    for content_idx in range(NUM_CONTENTS_TO_SEARCH):
        ...
        stat_s, stat_ar, stat_an_a, stat_an_b = gather_stats_for_content(content_idx, single_all_runs, attrib_reset_all_runs, attrib_noreset_all_adapt, attrib_noreset_all_base)
        plot_four_metrics_for_content(stat_s, stat_ar, stat_an_a, stat_an_b, content_label=content_idx+1)
    # 総合結果の統計量を計算してプロット
    overall_stats = {
        ...
        'NR-Base': gather_overall_stats(attrib_noreset_all_base)
    }
