...
def apply_icn_update(cache_storage, net_vector_arrays, cid, vect, start_node, path_from_found_to_start):
    ...
    # 3. マップの重み付け更新
    # cache_prop と同様に、ゴール(要求元)に近づくほど alpha を大きくする
        ...
        for map_index in range(NUM_MAPS):
            ...
            if found:
                ...
                if hops_used < min_hops_in_task:
                    ...
                    best_path_in_task = searched_node[::-1]
            else:
            ...
            for _ in range(NUM_ANTS):
                ...
                if found:
                    ...
                    if cost < global_min_hops_in_task:
                        ...
                        global_best_path_in_task = path[::-1]
                else:
...
def main():
    ...
    for _sim in range(TIME_TO_SIMULATE):
        ...
        for map_index in range(NUM_MAPS):
            ...
            current_map_vector = vectors_for_single[map_index]
            current_pheromone_dict = single_pheromone_dict_list[map_index]

            # コピーした環境 (cache_for_single, vectors_for_single) を渡す
            single_res = multi_contents_single_pheromone_with_reset(
                cache_for_single, 
                vectors_for_single[map_index], 
                vectors_for_single, # リスト全体もコピー版を渡す
                size, content_tasks, debug_logs, current_pheromone_dict
            )
            
            # コピーした環境 (cache_for_attrib, vectors_for_attrib) を渡す
            attrib_res = multi_contents_attrib_pheromone_no_reset(
                cache_for_attrib, 
                vectors_for_attrib[map_index], 
                vectors_for_attrib, 
                size, content_tasks, debug_logs
            )
        ...
        # コピーした環境 (cache_for_existing, vectors_for_existing) を渡す
