...
#iter=1を山白法,iter>1以降でSOMとACOの組み合わせを実施
#これは不採用
...
# --- 動的ALPHA/BETAパラメータ ---
# t=1 (ACOの1回目) の時の値
ALPHA_START = 1.0   # 最初はフェロモンの影響を 1.0 (または 0.5 など低め) に
BETA_START  = 5.0   # ★★★ 最初はSOMの影響を 5.0 など高めに ★★★

# t=最後 の時の値
ALPHA_END   = 1.0   # 最終的にフェロモンの影響を 1.0 に
BETA_END    = 1.0   # 最終的にSOMの影響を 1.0 に
Q = 100
                    ...
                    for _ in range(TIMES_TO_SEARCH_HOP):
                        ...
                        if  USE_EPSILON and random.random() < EPSILON:
                            ...
                            for n in allowed:
                                ...
                                eta = 1.0 / (dist + 1e-6)
                                if np.sum(vect) > 0:
                                    # ★★★ 変更点B: ALPHA を current_alpha に変更 ★★★
                                    A_ij = np.sum((tau_list ** current_alpha) * vect) / np.sum(vect)
                                else:
                                    A_ij = 0
                                
                                # ★★★ 変更点B: BETA を current_beta に変更 ★★★
                                score = A_ij * (eta ** current_beta)
...
def plot_overall_metrics(stats_dict):
    ...
    plt.figure()
    plt.plot(its, stats_dict['Single'][3], label='提案手法1(ID毎)', color='red', marker='o')
    plt.plot(its, stats_dict['NR-Adapt'][3], label='提案手法2(属性毎)', color='blue', marker='s')
    ...
    plt.figure()
    plt.plot(its, stats_dict['Single'][4], label='提案手法1(ID毎)', color='red', marker='o')
    plt.plot(its, stats_dict['NR-Adapt'][4], label='提案手法2(属性毎)', color='blue', marker='s')
