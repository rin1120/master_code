####### compare_search_methods_with_attribute_pheromones.py #######
##多次元フェロモンと既存手法の探索
...
def search_prop_with_aco(cache_storage, net_vector_array, size, search_tasks):
    ...
    for search_iter, (id, start_node) in enumerate(search_tasks):
            ...
            for ant in range(num_ants):
                ...
                while hops < max_hops:
                    ...
                    for neighbor in allowed_neighbors:
                        ...
                        # 属性ごとの評価値の計算
                        attribute_scores = (tau_list ** alpha) * vect  # vect = R_k
        ...
        #現在は、イテレーション終了ごとにフェロモンを揮発
        if all_paths:
            # フェロモンの蒸発（検索タスクごとに一度だけ行う）、現在はイテレーション終了ごとに揮発
...
def main():
    ...
    plt.title('Comparison of Average Hops between Original, ACO Searches, and Theoretical Minimum ')
    plt.legend()
    plt.grid(True)
    plt.show()
