...
# ----------------------------------------------------------------------------
TIME_TO_SIMULATE = 3
NUM_CONTENTS_TO_SEARCH = 10 # 10個のコンテンツを探索、この値を変更してください
NUM_ANTS = 10
NUM_ITERATIONS = 100
...
def gather_stats_for_content(content_index, single_all_runs, attrib_reset_all_runs, attrib_noreset_all_runs):
    """
    single_all_runs 等を引数に取り、content_indexに対応する統計量を取得。
    """
...
def plot_three_metrics_for_content(stat_single, stat_attrib_reset, stat_attrib_noreset, content_label):
    ...
    # stat_* は (medians, q1s, q3s, means, success_rates) のタプル
...
# ----------------------------------------------------------------------------
# average_iteration_data_across_runs 関数
# ----------------------------------------------------------------------------
def average_iteration_data_across_runs(all_runs, content_index=0):
    """
    all_runs: [ [iteration_data_of_content0, iteration_data_of_content1, ...], ... ]
              の形を想定している (single_all_runs等)
    content_index: 取得したいコンテンツのインデックス
    戻り値: (medians, q1s, q3s, means, success_rates)
    """
    iteration_data_runs = []
    for run_data in all_runs:  
        # run_data: [(NUM_ITERATIONS, NUM_ANTS), (NUM_ITERATIONS, NUM_ANTS), ...] (各コンテンツ)
    ...
    # 典型的には num_iterations = NUM_ITERATIONS
    ...
    for it in range(num_iterations):
        ...
        for one_run in iteration_data_runs:
            merged_costs.extend(one_run[it])  # 1イテレーション分のコストを結合
        combined_iteration_data.append(merged_costs)

    # まとめた(イテレーションごとの全コスト群)を compute_stats_from_costs で統計量取得
