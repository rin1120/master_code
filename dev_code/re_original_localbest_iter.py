...
REPORT_ITERS = 10  # 計算・表示はここまで（学習自体は NUM_ITERATIONS まで実施）
...
def generate_multi_contents_tasks_z(size, k=NUM_CONTENTS_TO_SEARCH, popularity_ratio=0.01, request_concentration=0.8, mode="unpopular_only"):
    tasks = []
...
def generate_multi_contents_tasks_s(size, k=NUM_CONTENTS_TO_SEARCH, burst_size=1000):
    """
    アンカーを1つ選び、そのコサイン類似度上位M(burst_size)から
    類似度に比例した重みでk件サンプリングして要求列を作る。
    既存の start_node ロジックはそのまま。
    """
    # ── 正規化済み行列を初回だけ作成（cos類似度 = 内積）
    if not hasattr(generate_multi_contents_tasks_s, "_Vn"):
        V = np.vstack(cont_vector_array)  # (cont_num, N)
        generate_multi_contents_tasks_s._Vn = V / (np.linalg.norm(V, axis=1, keepdims=True) + 1e-12)
    Vn = generate_multi_contents_tasks_s._Vn

    # アンカーを1つ選択
    anchor_idx = np.random.randint(cont_num)  # 0-based
    anchor_vec = Vn[anchor_idx]

    # 全件との類似度（内積）を計算して自分は除外
    sim = Vn @ anchor_vec
    sim[anchor_idx] = -np.inf

    # 上位Mを候補に（Mは範囲内にクリップ）
    M = int(max(1, min(burst_size, cont_num - 1)))
    idx_top = np.argpartition(sim, -M)[-M:]
    idx_top = idx_top[np.argsort(sim[idx_top])[::-1]]  # 類似度降順
    cand_cids = (idx_top + 1).astype(int)

    # 類似度→確率（負値は0、総和0なら一様）
    w = np.maximum(sim[idx_top], 0.0)
    probs = (w / w.sum()) if w.sum() > 0 else np.full(len(w), 1.0 / len(w))

    # CIDを一気にk個サンプル（重複可）
    sampled_cids = np.random.choice(cand_cids, size=k, replace=True, p=probs)
    ...
    for cid in sampled_cids:
        ...
        tasks.append((int(cid), start_node))
    return tasks
