...
def compute_stats_from_costs(iteration_costs_data):
    # iteration_costs_data: (NUM_ITERATIONS, NUM_ANTS) のコストデータ
    medians = []
    q1s = []
    q3s = []
    means = []
...
def main():
    ...
    # 同じ環境・同じタスクを使用して、TIME_TO_SIMULATE回実行
    ...
    def average_over_runs(data_list):
        data_array = np.array(data_list)  # shape: (TIME_TO_SIMULATE, NUM_ITERATIONS)
        return np.mean(data_array, axis=0)  # イテレーション方向に平均
