# ----------------------------------------------------------------------------
# オリジナル版 既存手法、ACO３手法の比較（フェロモン付加上限追加版）
# ----------------------------------------------------------------------------
import numpy as np
import pandas as pd
import random
            ...
            for _ in range(NUM_ANTS):
                ...
                for _ in range(TIMES_TO_SEARCH_HOP):
                    if cid in cache_storage[current_node[0]][current_node[1]]:
                        found = True
                        break
                    neighbor_list = get_neighbors(current_node[0], current_node[1], size)
                    allowed = [n for n in neighbor_list if n not in visited]
                    if not allowed:
                        break
                    probs = []
                    denom = 0
            ...
            for path, cost in zip(all_paths, all_costs):
                if cost > 0:
                    ...
                    for i in range(len(path) - 1):
                        ...
                        pheromone_trails[edge] += delta
                        if not reset_pheromone:
                            pheromone_trails[edge] = np.minimum(pheromone_trails[edge], PHEROMONE_MAX)
            iteration_data.append(iter_costs)
        results.append(iteration_data)
    return results
