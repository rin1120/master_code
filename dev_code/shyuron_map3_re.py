# ----------------------------------------------------------------------------
# Experiment 2 (Appendix): Dynamic cache update with multi-maps
# - Cache is SHARED among maps (common cache storage)
# - Search is performed on ALL maps in parallel, and we use the BEST (minimum hops) result
# - Compare UPDATE policy:
#     (1) COMPAT-UPDATE: update ONLY the most compatible map at start node
#     (2) RANDOM-UPDATE: update ONLY one randomly chosen map
# - Update happens only after SUCCESS:
#     * cache copy to start node (shared cache)
#     * SOM weight update along the found path (only on selected update map)
# - Metrics: average hops (failure treated as TIMES_TO_SEARCH_HOP)
...
# ----------------------------------------------------------------------------
# Plot style: force font size = 18 everywhere
# ----------------------------------------------------------------------------
plt.rcParams.update({
    "font.size": 18,
    "axes.titlesize": 18,
...
# ----------------------------------------------------------------------------
# Parallel search on ALL maps, take best hops result
# ----------------------------------------------------------------------------
def search_on_all_maps_ID(cache_storage, maps, size, cid, start, single_pher_list):
    best_h = TIMES_TO_SEARCH_HOP
    best_path = None
    best_map_idx = None

    for mi in range(len(maps)):
        path, h = run_single_task_single_pheromone(
            cache_storage, maps[mi], size, cid, start, single_pher_list[mi]
        )
        if h < best_h:
            best_h = h
            best_path = path
    ...
    return best_map_idx, best_path, best_h

def search_on_all_maps_ATTR(cache_storage, maps, size, cid, start, attrib_pher_list):
    best_h = TIMES_TO_SEARCH_HOP
    best_path = None
    best_map_idx = None

    for mi in range(len(maps)):
        path, h = run_single_task_attrib_pheromone(
            cache_storage, maps[mi], size, cid, start, attrib_pher_list[mi]
        )
        if h < best_h:
            best_h = h
            best_path = path
...
def run_one_setting(size, num_maps=NUM_MAPS_FIXED, seed=None):
    ...
    for _sim in range(TIME_TO_SIMULATE):
        ...
        for policy in ["COMPAT", "RANDOM"]:
            for method in ["ID", "ATTR"]:
                envs[(policy, method)] = {
                    ...
                    "single_pher": [{} for _ in range(num_maps)],  # per map: dict[cid]->trails
        ...
        for cid, start in tasks:
            ...
            # For fairness: the SEARCH outcome should be the same between update policies.
            # So we run parallel search separately per method, using each env's pheromones/maps.
            for method in ["ID", "ATTR"]:
                # compute SEARCH result on each policy's environment independently
                # (because pheromone/maps evolve by its own update policy)
                for policy in ["COMPAT", "RANDOM"]:
                    e = envs[(policy, method)]

                    # 1) parallel search on ALL maps and take BEST hops
                    if method == "ID":
                        search_map_idx, best_path, best_h = search_on_all_maps_ID(
                            e["cache"], e["maps"], size, cid, start, e["single_pher"]
                        )
                    else:
                        search_map_idx, best_path, best_h = search_on_all_maps_ATTR(
                            e["cache"], e["maps"], size, cid, start, e["attrib_pher"]
                        )

                    # 2) choose UPDATE map by policy (this is the only compared factor)
                    if best_path is not None:
                        if policy == "COMPAT":
                            update_map_idx = select_map_compatible(e["maps"], start, vect)
                        else:
                            update_map_idx = select_map_random(num_maps)

                        # 3) update ONLY that selected map, using the found best path
...
def main():
    ...
    out_csv = f"summary_update_policy_maps{NUM_MAPS_FIXED}_parallelSearch_bestUsed.csv"
