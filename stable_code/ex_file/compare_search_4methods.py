...
## ACOの各シミュレーションのホップ数は検索回数における最良経路のホップ数（1回の蟻の数＊イテレーション数中で最良の経路）
import numpy as np
import pandas as pd
import random
...
###   蟻コロニー最適化を用いたコンテンツの探索（ベクトル類似度を考慮）   ###
def search_prop_with_aco(cache_storage, net_vector_array, size, search_tasks):
    # ACOのパラメータ
    num_ants = 5  # 蟻の数
    num_iterations = 5  # イテレーション数
    alpha_initial = 1  # フェロモンの初期重要度
    alpha_final = 10.0    # フェロモンの最終重要度
    beta_initial = 10.0   # ヒューリスティック情報の初期重要度
    ...
    for search_iter, (id, start_node) in enumerate(search_tasks):
        ...
        # フェロモンレベルを1.0に初期化（各コンテンツの探索ごとに初期化）
...
###   単純な蟻コロニー最適化を用いたコンテンツの探索（フェロモン情報のみ使用）   ###
...
def main():
    ...
    for size in network_sizes:
        ...
        for i in range(TIME_TO_SIMULATE):
            ...
            cache_hit_simple_aco, cache_hit_rate_simple_aco, average_hops_simple_aco, average_theoretical_hops_simple_aco = search_prop_with_simple_aco(
                cache_storage, net_vector_array, size, search_tasks)
    ...
    TITLE_FONT_SIZE = 24   # タイトルのフォントサイズ
    LEGEND_FONT_SIZE = 33  # 凡例のフォントサイズ

    # グラフの作成
    plt.figure(figsize=(10, 6))
    plt.plot(network_sizes, original_average_hops_list, label='Original Search', marker='o')
    plt.plot(network_sizes, aco_average_hops_list, label='Propsal Search', marker='s')
    plt.plot(network_sizes, simple_aco_average_hops_list, label='Simple ACO', marker='D')
    plt.plot(network_sizes, theoretical_hops_list, label='Theoretical Minimum', marker='^')
    plt.xlabel('Network Size (Grid Length)', fontsize=X_AXIS_FONT_SIZE)
    plt.ylabel('Average Hops', fontsize=Y_AXIS_FONT_SIZE)
    #plt.title('Comparison of Average Hops among Different Search Methods by Network Size', fontsize=TITLE_FONT_SIZE)
    plt.legend(fontsize=LEGEND_FONT_SIZE)

    # 軸の目盛りとラベルをカスタマイズ
    plt.xticks(network_sizes, fontsize=TICKS_FONT_SIZE)
    max_y_value = max(original_average_hops_list + aco_average_hops_list + simple_aco_average_hops_list + theoretical_hops_list)
    step_size = 5
    max_y_value_rounded = math.ceil(max_y_value / step_size) * step_size
    y_ticks = np.arange(0, max_y_value_rounded + step_size, step_size)
    plt.yticks(y_ticks, fontsize=TICKS_FONT_SIZE)
