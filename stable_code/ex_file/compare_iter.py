####### compare_iterations_and_network_sizes_with_aco.py #######
import numpy as np
import pandas as pd
import random
...
###   理論最短ホップ数の計算   ###
def compute_theoretical_hops(size, search_tasks, cache_storage):
    ...
    return average_theoretical_hops
...
def main():
    ...
    iteration_counts = [10, 20, 30, 40, 50]  # イテレーション数を変化させる
    ...
    for size in network_sizes:
        ...
        for i in range(TIME_TO_SIMULATE):
            # キャッシュ配置とネットワークベクトルを生成（各シミュレーション回ごとに一度だけ）
            cache_storage, net_vector_array = cache_prop(size)
            # 検索タスクの生成
            search_tasks = generate_search_tasks(size)

            # 理論最短ホップ数を計算（イテレーション数に依存しないため、一度だけ計算）
            theoretical_hops = compute_theoretical_hops(size, search_tasks, cache_storage)
            temp_theoretical_hops.append(theoretical_hops)

            for num_iterations in iteration_counts:
                # ACOアルゴリズムによる探索（同じキャッシュ配置と検索タスクを使用）
                cache_hit_aco, cache_hit_rate_aco, average_hops_aco, _ = search_prop_with_aco(
                    cache_storage, net_vector_array, size, search_tasks, num_iterations)

                # 結果を保存
                key = (size, num_iterations, i)
                results[key] = average_hops_aco

                print(f"--- シミュレーション {i+1}, イテレーション数: {num_iterations} ---")
                print(f"キャッシュヒット数: {cache_hit_aco}")
                print(f"キャッシュヒット率: {cache_hit_rate_aco} %")
                print(f"平均ホップ数: {average_hops_aco}")
                print(f"理論最短ホップ数: {theoretical_hops}")

        # 各ネットワークサイズでの理論最短ホップ数の平均を計算
    ...
    for num_iterations in iteration_counts:
        ...
        for size in network_sizes:
            ...
            for i in range(TIME_TO_SIMULATE):
                key = (size, num_iterations, i)
                temp_hops.append(results[key])
            average_hops = sum(temp_hops) / len(temp_hops)
            average_hops_list.append(average_hops)
    ...
    plt.title('Effect of Iterations on Average Hops in ACO Search')
    plt.legend()
    plt.grid(True)
    plt.show()
