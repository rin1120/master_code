####### compare_search_methods_with_shared_tasks.py #######
import numpy as np
import pandas as pd
import random
...
def search_prop_with_aco(cache_storage, net_vector_array, size, search_tasks):
    ...
    return cache_hit, cache_hit_rate, average_hops
...
def main():
    ...
    for size in network_sizes:
        ...
        for i in range(TIME_TO_SIMULATE):
            ...
            # オリジナルの探索
            cache_hit, cache_hit_rate, average_hops = search_prop(cache_storage, net_vector_array, size, search_tasks)
            ...
            # ACOアルゴリズムによる探索
            cache_hit_aco, cache_hit_rate_aco, average_hops_aco = search_prop_with_aco(cache_storage, net_vector_array, size, search_tasks)
    ...
    plt.title('Comparison of Average Hops between Original and ACO Searches by Network Size')
    plt.legend()
    plt.grid(True)
    plt.show()
